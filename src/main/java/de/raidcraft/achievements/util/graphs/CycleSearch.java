...
import io.artframework.util.graphs.AdjacencyList;
import io.artframework.util.graphs.SCCResult;
import io.artframework.util.graphs.StrongConnectedComponents;
...
public class CycleSearch<TObject> {
	...
	/**
	 * Creates a new cycle searcher that will search the dependency graph of the achievements
	 * and returns a list of achievements that have cyclic dependencies.
	 * <p>
	 * Use the {@link #getCycles()} method to get a list of all achievements that have cyclic depencencies.
	 *
	 * @param achievements list of achievements to create dependency graph for
	 * @return the dependency graph of the achievements
	 */
	public static CycleSearch<Achievement> of(Collection<Achievement> achievements) {
		Achievement[] array = achievements.toArray(new Achievement[0]);
		return new CycleSearch<>(getAdjacencyMatrix(array), array);
	}

	public static String dependencyGraphToString(@NonNull List<? extends Object> graph) {

		Object achievement = graph.get(0);
		if (graph.size() < 2) return achievement + " depends on itself!";
	...
	private static boolean[][] getAdjacencyMatrix(Achievement[] modules) {
	...
	private static Integer[] getDependencyMatrix(Achievement[] achievements, Achievement achievement) {

		List<Integer> dependencyIdicies = new ArrayList<>();

		for (Achievement child : achievement.children()) {
			for (int i = 0; i < achievements.length; i++) {
				Achievement parent = achievements[i];
